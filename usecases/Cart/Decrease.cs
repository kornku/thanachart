...
public record DecreaseCartCommand(Guid id,List<CartProductResponse> list) : ICommand<Result<bool>>;

public class DecreaseCartHandler
    ( IRepository<CartEntity> cartRepository,IRepository<CartProductEntity> cartProductRepository,
        IRepository<ProductEntity> productRepository) : ICommandHandler<DecreaseCartCommand,
        Result<bool>>
{
    public async Task<Result<bool>> Handle(DecreaseCartCommand request, CancellationToken cancellationToken)
    {
